...
# Pause inserted right after a URL, regardless of what follows. 
# Comma-ish weight: a real breath, but not necessarily a full sentence end.
URL_PAUSE_MS = 250
...
# stray bit of punctuation on its own (not sure how useful this is)
MIN_CHUNK_CHARS = 12
...
# is a safety valve for giant unpunctuated pastes, not a "correct" split
LONG_CHUNK_WORD_LIMIT = 100
FORCED_SPLIT_PAUSE_MS = 0 # Was 80. Probably better as 0.
...
def _humanize_url(raw: str) -> str:
    ...
    the "http" or "https" part as well as  a few URL punctuations like
    ".:/_-~" as dot, colon, slash, underline, dash, tilde respectively."""
    core = raw
    if core.startswith("<"):
        core = core[1:]
    if core.endswith(">"):
    ...
    core = core.replace(".", " dot ").replace("/", " slash ")
    core = core.replace(":", " colon ").replace("_", " underline ")
    core = core.replace("-", " dash ").replace("~", " tilde ")
    #print (re.sub(r"\s+", " ", core).strip())
...
def _map_source_offset_to_local(chunk: Chunk, offset: int) -> Optional[int]:
    """Maps a source-text offset to the corresponding local index inside
    `chunk.text`, via `chunk.text_ranges` -- or None if `offset` doesn't
    fall strictly inside one of `chunk`'s spans (i.e. it's already right
    on a token boundary, or outside the chunk entirely)."""
    for (s, e), (ts, te) in zip(chunk.spans, chunk.text_ranges):
        if s < offset < e:
            return ts + (offset - s)
    return None
...
def _first_force_break_local_idx(chunk: Chunk, force_breaks: set) -> Optional[int]:
    """Returns the local text index of the earliest `force_breaks` offset
    that falls inside `chunk`'s source span, or None if there isn't one."""
    if not chunk.spans:
        return None
    lo, hi = chunk.start, chunk.end
    candidates = [b for b in force_breaks if lo < b < hi]
    if not candidates:
        return None
    return _map_source_offset_to_local(chunk, min(candidates))


def _apply_force_breaks(chunks: List[Chunk], force_breaks: set) -> List[Chunk]:
    """Splits any chunk whose source span straddles one of `force_breaks`
    (character offsets marking an implied sentence boundary that isn't
    backed by actual punctuation -- e.g. a font-size change) into two
    chunks at that point. The left piece is given a full sentence-ending
    pause, same weight as a period, so it's treated by everything
    downstream (merging, TTS pacing) exactly like a real sentence end."""
    if not force_breaks:
        return chunks
    out: List[Chunk] = []
    for c in chunks:
        ...
        while queue:
            cur = queue.pop(0)
            idx = _first_force_break_local_idx(cur, force_breaks)
            if idx is None or idx <= 0 or idx >= len(cur.text):
                out.append(cur)
                continue
            left, right = _split_chunk_at(cur, idx)
            if not left.spans or not right.spans:
                out.append(cur)
                continue
            left.pause_ms = max(left.pause_ms, PAUSE_MAP["."])
            out.append(left)
            queue.insert(0, right)
    return out


def chunk_text(text: str, force_breaks: Optional[set] = None) -> List[Chunk]:
    """Split `text` into a list of Chunk objects in reading order.

    `force_breaks`, if given, is a set of character offsets into `text`
    where a sentence-level break should be treated as present even though
    no punctuation marks it there (used by the GUI for font-size-change
    detection). Applied right after tokenizing, before the short-fragment
    merge pass, so a forced break behaves exactly like a real one from
    that point on."""
    raw = (_strip_chunk(c) for c in _tokenize(text))
    raw = [c for c in raw if c.text]
    if force_breaks:
        raw = _apply_force_breaks(raw, force_breaks)
